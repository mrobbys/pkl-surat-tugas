...
<head>
    ...
    <meta http-equiv="X-UA-Compatible"
        content="ie=edge">
    <title>Rekapitulasi Surat Telaahan Staf</title>
    <style>
        /* Reset & Base */
        * {
...
<body>
    ...
    {{-- Judul Laporan --}}
    <h1 class="report-title">Rekapitulasi Surat Telaahan Staf</h1>
        ...
        <tbody>
            @forelse($data as $index => $item)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>
                        {{ \Carbon\Carbon::parse($item->tanggal_telaahan)->translatedFormat('d M Y') }}
                    </td>
                    <td>{{ $item->nomor_telaahan }}</td>
                    <td>{{ Str::limit($item->perihal_kegiatan, 60) }}</td>
                    <td>{{ $item->kepada_yth }}</td>
                    <td>{{ $item->pembuat->nama_lengkap ?? '-' }}</td>
                    <td>
